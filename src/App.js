...
async function dbSet(data) {
  ...
  // Fallback: GET con chunks — divide en jugadores y pronósticos por separado
  try {
    // Guarda solo players+results+globalLocked (pequeño)
    const small = { players: data.players, results: data.results, globalLocked: data.globalLocked };
    const enc1 = encodeURIComponent(JSON.stringify(small));
    if (enc1.length < 7000) {
      // Cabe en URL, guarda todo
      const enc = encodeURIComponent(json);
      if (enc.length < 7000) {
        await fetch(`${API}?action=set&data=${enc}`);
        return true;
      }
    }
    // Si es muy grande, guarda sin pronósticos primero (al menos salva el registro)
    await fetch(`${API}?action=set&data=${encodeURIComponent(JSON.stringify(small))}`);
    return true;
  } catch(e) {
    console.error("dbSet fallback error:", e);
    return false;
  }
}
