...
const DEADLINE = new Date("2026-06-11T05:00:00Z"); // 10 jun 23:00 CDMX
const isPastDeadline = () => new Date() >= DEADLINE;

// ── PARTIDOS ORDENADOS POR FECHA — fuente ESPN oficial ─────────────────────
// Horarios en CDMX (UTC-5 en verano)
const ALL_MATCHES = [
  // 11 Jun
  ...
  // 24 Jun
  { id:49, grp:"B", local:"Suiza",           visit:"Canada",            date:"24 Jun", time:"13:00", venue:"BC Place, Vancouver" },
  { id:50, grp:"B", local:"Bosnia y Herz.",  visit:"Catar",             date:"24 Jun", time:"13:00", venue:"AT&T, Dallas" },
  { id:51, grp:"A", local:"Mexico",          visit:"Rep. Checa",        date:"24 Jun", time:"20:00", venue:"Azteca, CDMX" },
  { id:52, grp:"A", local:"Sudafrica",       visit:"Corea del Sur",     date:"24 Jun", time:"20:00", venue:"BBVA, Monterrey" },
  // 25 Jun
  { id:53, grp:"D", local:"EE.UU.",          visit:"Turquia",           date:"25 Jun", time:"13:00", venue:"SoFi, Los Angeles" },
  { id:54, grp:"D", local:"Paraguay",        visit:"Australia",         date:"25 Jun", time:"13:00", venue:"Levi's, San Francisco" },
  { id:55, grp:"C", local:"Brasil",          visit:"Escocia",           date:"25 Jun", time:"16:00", venue:"MetLife, Nueva Jersey" },
  { id:56, grp:"C", local:"Marruecos",       visit:"Haiti",             date:"25 Jun", time:"16:00", venue:"Mercedes-Benz, Atlanta" },
  // 26 Jun
  { id:57, grp:"E", local:"Alemania",        visit:"Ecuador",           date:"26 Jun", time:"11:00", venue:"Lincoln, Philadelphia" },
  { id:58, grp:"E", local:"Curazao",         visit:"Costa de Marfil",   date:"26 Jun", time:"11:00", venue:"NRG, Houston" },
  { id:59, grp:"F", local:"Paises Bajos",    visit:"Tunez",             date:"26 Jun", time:"14:00", venue:"AT&T, Dallas" },
  { id:60, grp:"F", local:"Japon",           visit:"Suecia",            date:"26 Jun", time:"14:00", venue:"Arrowhead, Kansas City" },
  { id:61, grp:"G", local:"Belgica",         visit:"Nueva Zelanda",     date:"26 Jun", time:"19:00", venue:"Lumen Field, Seattle" },
  { id:62, grp:"G", local:"Egipto",          visit:"Iran",              date:"26 Jun", time:"19:00", venue:"BC Place, Vancouver" },
  { id:63, grp:"H", local:"Espana",          visit:"Uruguay",           date:"26 Jun", time:"22:00", venue:"Akron, Guadalajara" },
  { id:64, grp:"H", local:"Cabo Verde",      visit:"Arabia Saudita",    date:"26 Jun", time:"22:00", venue:"Hard Rock, Miami" },
  // 27 Jun
  { id:65, grp:"I", local:"Francia",         visit:"Noruega",           date:"27 Jun", time:"13:00", venue:"Gillette, Boston" },
  { id:66, grp:"I", local:"Senegal",         visit:"Iraq",              date:"27 Jun", time:"13:00", venue:"BMO Field, Toronto" },
  { id:67, grp:"J", local:"Argentina",       visit:"Jordania",          date:"27 Jun", time:"16:00", venue:"AT&T, Dallas" },
  { id:68, grp:"J", local:"Argelia",         visit:"Austria",           date:"27 Jun", time:"16:00", venue:"Arrowhead, Kansas City" },
  ...
  { id:71, grp:"L", local:"Inglaterra",      visit:"Panama",            date:"27 Jun", time:"22:00", venue:"MetLife, Nueva Jersey" },
  { id:72, grp:"L", local:"Croacia",         visit:"Ghana",             date:"27 Jun", time:"22:00", venue:"Lincoln, Philadelphia" },
];
...
function countCaptured(playerPreds) {
  ...
  for (const m of ALL_MATCHES) {
    ...
    if (isMex(m)) { if (Array.isArray(pred) && pred[0]!=="" && pred[1]!=="") count++; }
    else { if (pred && pred !== "") count++; }
  }
  return count;
}
...
function generateComprobante(playerName, playerPreds, results, lockedAt) {
  ...
  for (const [grp, matches] of Object.entries(byGroup)) {
    ...
    for (const m of matches) {
      ...
      lines.push("  "+m.date+" "+m.time+" — "+m.local+" vs "+m.visit+mxTag);
  ...
  lines.push("=".repeat(52));
  lines.push(""); lines.push("Comprobante oficial — Quiniela Mundial 2026 — " + lockedAt);
...
.match-venue{font-size:10px;color:var(--muted);margin-bottom:6px;}
...
export default function App() {
  ...
  const handleRegister = async () => {
    if (deadlinePassed) { setPErr("El torneo ya inicio, no es posible registrarse"); return; }
    ...
    setSession({ name }); setShowPlayerModal(false);
  ...
  const lockMyPreds = async () => {
    ...
    if (captured < TOTAL_MATCHES) {
      showToast("Necesitas capturar los 72 partidos. Te faltan " + (TOTAL_MATCHES - captured) + ".", "warn");
      return;
    }
    if (!window.confirm("Cerrar tus pronosticos? Ya capturaste los 72 partidos.\nSe descargara tu comprobante.\nSolo el admin puede reabrirlos si te equivocaste.")) return;
  ...
  // Agrupar partidos por fecha para mostrar
  const matchesByDay = {};
  for (const m of ALL_MATCHES) {
                ...
                <div className="prog-wrap">
                  ...
                  {myCaptured < TOTAL_MATCHES && !myLocked && (
                    <div style={{fontSize:11,color:"var(--muted)",marginTop:6}}>
                      Necesitas capturar los 72 partidos para poder cerrar tus pronosticos.
                    </div>
                  )}
                </div>
                <div className="sbar">
            ...
            {Object.entries(matchesByDay).map(([day, dayMatches]) => (
              <div key={day} className="day-block">
                <div className="day-header">
                  ...
                  <span className="day-grps">
                    {[...new Set(dayMatches.map(m=>"Grupo "+m.grp))].join(" · ")}
                  </span>
                </div>
        ...
        {tab==="info" && (
          <>
            ...
            <div className="rules-box">
              ...
              <div className="rule-row"><span>2️⃣</span><div>Captura tus pronosticos en los <strong>72 partidos</strong> antes del <strong>10 de junio</strong>. Es obligatorio llenarlos todos para poder cerrar.</div></div>
              <div className="rule-row"><span>3️⃣</span><div>Con los 72 partidos listos toca <strong>Cerrar y descargar PDF</strong>. Se guarda tu comprobante con fecha y hora.</div></div>
                        ...
                        <div className="pmgmt-left">
                          ...
                          <div>
                            ...
                            <div className={`pmgmt-status ${p.locked?"lk":"ok"}`}>
                              {p.locked?"Cerrado"+(p.lockedAt?" — "+p.lockedAt:""):"Abierto — "+p.captured+"/"+TOTAL_MATCHES+" capturados"}
                            </div>
                          </div>
                        </div>
